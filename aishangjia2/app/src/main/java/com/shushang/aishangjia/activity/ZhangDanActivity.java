...
import com.shushang.aishangjia.activity.adapter.ZhangDanAdapter2;
...
public class ZhangDanActivity extends BaseActivity implements BaseQuickAdapter.RequestLoadMoreListener,SwipeRefreshLayout.OnRefreshListener {
    ...
    private ZhangDanAdapter2 mZhangDanAdapter2;
    ...
    private String leagueFinanceId=null;
    ...
    public void init() {
        ...
        String quanxian = PreferencesUtils.getString(ZhangDanActivity.this, "quanxian");
...
//        mZhangDanAdapter2 = new ZhangDanAdapter2(R.layout.item_zhangdan, dataList,mHandler);
            ...
            if(type.equals("7")){
                if(mQuanXians.size()!=0){
                    for (int i = 0; i < mQuanXians.size(); i++) {
                        if(mQuanXians.get(i).equals("2216")){
                            mZhangDanAdapter.setOnItemChildLongClickListener(new BaseQuickAdapter.OnItemChildLongClickListener() {
                                @Override
                                public boolean onItemChildLongClick(BaseQuickAdapter adapter, View view, final int position) {
                                    switch (view.getId()){
                                        case R.id.ll_zhangdan:
                                            ExtAlertDialog.showSureDlg(ZhangDanActivity.this, "提醒", "确定退掉此单吗?", "确定", new ExtAlertDialog.IExtDlgClick() {
                                                @Override
                                                public void Oclick(int result) {
                                                    if(result==1){
                                                        leagueFinanceId=dataList.get(position).getLeagueFinanceId();
                                                        if(dataList.get(position).getEnable().equals("1")){
                                                            final String url= BaseUrl.BASE_URL+"phoneLeagueController.do?method=cancelFinance&token_id="+token_id+"&leagueFinanceId="+leagueFinanceId;
                                                            Log.d("quitUrl",url);
                                                            RestClient.builder()
                                                                    .url(url)
                                                                    .success(new ISuccess() {

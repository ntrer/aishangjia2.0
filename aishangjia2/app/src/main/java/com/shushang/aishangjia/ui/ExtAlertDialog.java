...
import com.shushang.aishangjia.ui.dialog.ItemAdapter;
...
//        });
    ...
    public static void showEditDlg2(Context context, String title, List<String> data,boolean isCancelable, final IExtDlgClick2 onclickListener) {
        final AlertDialog dlg = new AlertDialog.Builder(context).create();
        dlg.setView(new EditText(context));
        dlg.show();
        dlg.setContentView(R.layout.voice_dialog);
        dlg.getWindow().setBackgroundDrawableResource(
                android.R.color.transparent);
        View titleLayout = dlg.findViewById(R.id.title_layout2);
        TextView titleTxt = (TextView) dlg.findViewById(R.id.ext_dialog_title2);
        if (TextUtils.isEmpty(title))
            ...
            titleTxt.setText(title);
        final RecyclerView recyclerView =  dlg
                .findViewById(R.id.tishiyu);
        final LinearLayoutManager linermanager=new LinearLayoutManager(context);
        recyclerView.setLayoutManager(linermanager);
        ItemAdapter itemAdapter=new ItemAdapter(R.layout.item_tishi,data);

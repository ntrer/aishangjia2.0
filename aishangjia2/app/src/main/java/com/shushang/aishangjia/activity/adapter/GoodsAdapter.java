...
//    }
    ...
    protected void convert(BaseViewHolder helper, Good.DataListBean item) {
        ...
        if(item.getGoodsImages()!=null){
            ...
            String url=BaseUrl.BASE_URL+ "fileController.do?method=showimage&id="+split[0]+ "&token_id="+PreferencesUtils.getString(mContext,"token_id");
            Glide.with(mContext).load(url).override(ConvertUtils.dp2px(80),ConvertUtils.dp2px(80)).centerCrop().placeholder(R.mipmap.img).into(niceImageView);
        }

...
//        mCv = (CardView) findViewById(R.id.cv);
        ...
        tuisong=getIntent().getStringExtra("tuisong");
        initData();
    }

    private void initData() {
        if (PreferencesUtils.getString(mContext, "token_id")!=null&&PreferencesUtils.getString(mContext,"userId")!=null) {
//            checkLogin();
//            if(PreferencesUtils.getString(mContext,"type").equals("3")){
            Intent intent=new Intent(LoginActivity2.this, MainActivity2.class);
            if(tuisong!=null){
                if(tuisong.equals("推送")){
                    intent.putExtra("push","有推送");
...
//            final String user_name = AESCrypt.encrypt(mTxtUserName.getText().toString(),"");
        ...
        if(!company.equals("")&&!user_name.equals("")&&!password.equals("")){
           if(LoginActivity2.this!=null&&!LoginActivity2.this.isDestroyed()&&!LoginActivity2.this.isFinishing()){
            ...
            String url = BaseUrl.BASE_URL+"pcAndApploginController.do?method=login&username=" + user_name + "&code=" + company + "&password=" + md5(password) + "&loginOS=" + "1";
            Log.d("response", url);
            try {
                RestClient.builder()
                        .url(url)
                            ...
                            public void onSuccess(String response) {
                                ...
                                try {
                                    ...
                                    if(Integer.parseInt(login.getRet())==200){
                                        ...
                                        PreferencesUtils.putString(mContext,"pwd",password);
                                        PreferencesUtils.putString(mContext, "token_id",login.getData().getToken_id());
                                        PreferencesUtils.putString(mContext, "userId",login.getData().getUserId());
...
//                                        PreferencesUtils.putBoolean(mContext,"checked",true);
                                        Log.d("typesss",login.getData().getShangjia_id());
...
//                                            Log.d("leagueFlag",login.getData().getLeagueFlag());
...
//                                        }
    ...
    public void exit() {
        if ((System.currentTimeMillis() - mExitTime) > 2000) {
            ...
            PreferencesUtils.putBoolean(LoginActivity2.this,"isAlive",false);
            PreferencesUtils.putBoolean(LoginActivity2.this,"isclick",false);
